#!/usr/bin/env python3
"""
Script to revert motor_details_list in views.py to remove auto-complaint creation
"""
...
# The current (modified) POST handler code to revert
modified_post_handler = '''    elif request.method == 'POST':
        try:
...
# The original POST handler code
original_post_handler = '''    elif request.method == 'POST':
        try:
...
count = content.count(modified_post_handler)
print(f"Found {count} occurrences of modified POST handler")

if count > 0:
    new_content = content.replace(modified_post_handler, original_post_handler)
    
    # Write back
    with open('user/views.py', 'w', encoding='utf-8') as f:
        f.write(new_content)
    
    print(f"Successfully reverted {count} occurrences!")
else:
    print("Could not find the modified POST handler pattern")
